...
if [[ "$image_tag" == "e2e" ]]; then
  ...
  git clone https://github.com/skycoin/skycoin-service-discovery --depth 1 --branch "$git_branch" ./tmp/skycoin-service-discovery
  ...
  if [[ "$GITHUB_TOKEN" != "" ]]; then
    git clone https://"$GITHUB_TOKEN"@github.com/skycoin/skywire-ut --depth 1 --branch "$git_branch" ./tmp/skywire-ut
  else
